...
Lesson document output uses this serialized field order:
...
- `media` MUST be `null` when no resolved document media object is available
...
- `download_url` MUST NOT be emitted as contract output
- No additional document-specific resolver payload fields may be emitted
