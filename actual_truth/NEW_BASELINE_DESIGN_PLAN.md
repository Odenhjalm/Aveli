...
- Profile/community media and studio sessions are not protected-slot source tables inside the canonical baseline core defined here.
- If these features are added above the baseline, they must use explicit typed contracts and the same canonical media chain defined for core surfaces.
...
| `0003` | `course_enrollments_core.sql` | canonical `course_enrollments` table and drip invariants | membership logic, entitlement fallback |
| `0004` | `lessons_core.sql` | canonical `lessons` table and lesson ordering constraints | lesson body content, intro flags, lesson pricing, media state |
| `0005` | `lesson_contents_core.sql` | canonical `lesson_contents` table and lesson body content storage | structure fields, media linkage, fallback content storage |
| `0006` | `lesson_media_core.sql` | canonical `lesson_media` table and authored media placement constraints | storage shortcuts, hybrid legacy linkage |
| `0007` | `media_assets_core.sql` | canonical `media_assets` table and media-identity constraints | authored placement, runtime fallback |
...
| `0010` | `worker_query_support.sql` | canonical worker function plus worker-only indexes/views needed for canonical audio/drip execution | queue truth tables, alternate mutation paths, fallback business logic |
| `0011` | `course_public_content_core.sql` | canonical `course_public_content` table for learner-facing course descriptions | lesson structure, access logic, fallback copies |
| `0012` | `canonical_access_policies.sql` | `course_discovery_surface`, `lesson_structure_surface`, and `lesson_content_surface` policies using canonical authorities only | compatibility paths, mixed semantics |
...
- `media_purpose` must include `course_cover` and `lesson_media`, and may be append-only extended for additional governed media surfaces above the protected baseline range
...
- `purpose` must be a canonical `media_purpose` value
- protected core baseline rows must use `course_cover` or `lesson_media`
- append-only canonical extensions may add additional governed media purposes above the protected baseline range
...
### 2.7 `runtime_media` Unified Resolution Boundary

`runtime_media` is the unified runtime truth layer, implemented as a read-only projection boundary.

Projection laws:

- `runtime_media` is not source-of-truth storage.
- `runtime_media` is derived only from canonical source tables in `baseline_slots` and any append-only canonical extensions above the protected range.
- `runtime_media` must expose all governed media usages, not only lesson media.
- `runtime_media` must expose runtime truth for course cover, lesson media, home player, profile/community media, and future canonical media usages.
- A row is valid only when canonical source state authorizes deterministic runtime truth for state and resolution eligibility.
...
- The frontend-facing media object is constructed only by backend read composition as `media = { media_id, state, resolved_url } | null`.
...
- one projected row per canonical media usage
- canonical linkage back to:
  - the owning media usage record
  - `lesson`
  - `course`
  - `media_asset`
...
- no frontend representation logic
...
3. Ensure runtime resolution eligibility is based only on canonical source state.
...
- projection covers all canonical media surfaces
...
- old fallback-capable `runtime_media` -> new unified projection-only `runtime_media`
...
- `runtime_media` as unified media-resolution projection
- backend read composition as the only frontend media-representation authority
...
- no fallback media-resolution logic
