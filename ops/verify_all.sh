...
log "Remote DB verify (read-only) via backend/scripts/db_verify_remote_readonly.py"
if [[ "$APP_ENV_MODE" != "prod" ]]; then
  echo "Remote DB verify is non-blocking in ${APP_ENV_MODE} mode"
fi
remote_exit=1
if [[ ! -f "$MASTER_ENV_FILE" ]]; then
  echo "master.env missing at ${MASTER_ENV_FILE}" >&2
  remote_status="FAIL"
else
  if command -v poetry >/dev/null 2>&1; then
    if ensure_backend_deps; then
      set +e
      (
        load_env_file "$MASTER_ENV_FILE"
        cd "${ROOT_DIR}/backend"
        poetry run python scripts/db_verify_remote_readonly.py
      )
      remote_exit=$?
      set -e
    else
      echo "poetry install failed; remote DB verify failed" >&2
      remote_exit=1
    fi
  else
    echo "poetry not found; remote DB verify failed" >&2
    remote_exit=1
  fi
...
if [[ "$remote_status" == "FAIL" && "$APP_ENV_MODE" == "prod" ]]; then
  overall_status=1
fi

log "Local DB reset (optional)"
if [[ "${LOCAL_DB_RESET:-}" == "1" || "${AUTO_LOCAL_RESET:-}" == "1" ]]; then
  if [[ -x "${ROOT_DIR}/ops/db_repair_local_reset.sh" ]]; then
    set +e
    ...
    echo "db_repair_local_reset.sh missing; skipping local reset" >&2
    local_reset_status="SKIP"
  fi
else
  local_reset_status="SKIP"
