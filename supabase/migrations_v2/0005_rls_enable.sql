-- 0005_rls_enable.sql
-- Enable RLS and add service_role_full_access for all app tables.
...
drop policy if exists service_role_full_access on app.classroom_messages;
create policy service_role_full_access on app.classroom_messages
  for all using (auth.role() = 'service_role') with check (auth.role() = 'service_role');

alter table app.classroom_presence enable row level security;
drop policy if exists service_role_full_access on app.classroom_presence;
create policy service_role_full_access on app.classroom_presence
  for all using (auth.role() = 'service_role') with check (auth.role() = 'service_role');

alter table app.lesson_packages enable row level security;
drop policy if exists service_role_full_access on app.lesson_packages;
create policy service_role_full_access on app.lesson_packages
  for all using (auth.role() = 'service_role') with check (auth.role() = 'service_role');

alter table app.teacher_accounts enable row level security;
drop policy if exists service_role_full_access on app.teacher_accounts;
create policy service_role_full_access on app.teacher_accounts
  for all using (auth.role() = 'service_role') with check (auth.role() = 'service_role');

alter table app.welcome_cards enable row level security;
drop policy if exists service_role_full_access on app.welcome_cards;
create policy service_role_full_access on app.welcome_cards
  for all using (auth.role() = 'service_role') with check (auth.role() = 'service_role');

alter table app.music_tracks enable row level security;
drop policy if exists service_role_full_access on app.music_tracks;
create policy service_role_full_access on app.music_tracks
  for all using (auth.role() = 'service_role') with check (auth.role() = 'service_role');
