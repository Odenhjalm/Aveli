...
api_fix_payments_subscription

---

## PROBLEM

- Frontend calls: GET /payments/subscription
- Backend route is missing or mismatched for this path
- Equivalent handler expected in: `backend/app/routes/billing.py`, `backend/app/routes/api_checkout.py`, `backend/app/routes/api_me.py`
...
- Open route source files:
  `backend/app/routes/billing.py`, `backend/app/routes/api_checkout.py`, `backend/app/routes/api_me.py`
...
- return current user subscription state
- be read-only
- depend on authenticated user context
...
- be JSON
- include subscription status (`active` / `inactive` / `trial` / `canceled`)
- include plan identifier
- include current period or expiry information
...
- handler mutates subscription state
- handler creates or updates subscription state
...
- modify Stripe logic
- create or mutate subscription state
...
- Response includes required fields:
  - subscription status
  - plan identifier
  - current period / expiry information
...
- handler creates Stripe objects that modify state
