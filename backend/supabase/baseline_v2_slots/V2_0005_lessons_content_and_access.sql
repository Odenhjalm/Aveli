create table app.lessons (
  ...
  constraint lessons_lesson_title_not_blank_check check (btrim(lesson_title) <> '')
);

create index lessons_course_id_idx
  on app.lessons (course_id);

comment on table app.lessons is
  'Canonical lesson structure table. It contains lesson identity and ordering only, not lesson content or access authority.';

comment on column app.lessons.lesson_title is
  'Canonical lesson title for structure and read projections.';

comment on column app.lessons.position is
  'Explicit lesson order within a course. This does not define course progression authority.';
...
comment on table app.lesson_contents is
  'Canonical lesson content table. Visibility and access logic are intentionally outside this table.';

comment on column app.lesson_contents.content_markdown is
  'Authored lesson body content for protected lesson content reads.';
...
create table app.course_enrollments (
  ...
  constraint course_enrollments_user_id_fkey
  ...
  constraint course_enrollments_current_unlock_position_check
    check (current_unlock_position >= 0),
  constraint course_enrollments_drip_started_at_check
    check (drip_started_at = granted_at)
);

create index course_enrollments_user_id_idx
  on app.course_enrollments (user_id);

create index course_enrollments_course_id_idx
  on app.course_enrollments (course_id);

comment on table app.course_enrollments is
  'Canonical protected course-access state. Rows must be created only through app.canonical_create_course_enrollment.';

comment on column app.course_enrollments.source is
  'Canonical access source: purchase for sellable courses, intro_enrollment for non-sellable courses.';

comment on column app.course_enrollments.current_unlock_position is
  'Worker-controlled stored unlock position for protected lesson access.';
...
begin
  ...
  if new.current_unlock_position is distinct from old.current_unlock_position then
    if not in_worker_context then
      raise exception
        'current_unlock_position may be advanced only through the canonical worker context';
    end if;

    if new.current_unlock_position < old.current_unlock_position then
      raise exception
        'canonical unlock progression must never decrease current_unlock_position';
    end if;
  end if;
...
begin
  ...
  group by c.id, c.sellable, c.drip_enabled;
  ...
  if v_course_sellable = true
     and p_source <> 'purchase'::app.course_enrollment_source then
    raise exception
      'sellable courses require source = purchase';
  end if;

  if v_course_sellable = false
     and p_source <> 'intro_enrollment'::app.course_enrollment_source then
    raise exception
      'non-sellable courses require source = intro_enrollment';
  end if;
