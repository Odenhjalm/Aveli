...
async def inspect_user(user_id: str) -> dict[str, Any]:
    ...
    (
        ...
        profile_row,
        membership_row,
        authored_courses_raw,
        enrolled_courses_raw,
        entitlement_slugs_raw,
    ) = await asyncio.gather(
        auth_repo.get_user_by_id(normalized_user_id),
        profiles_repo.get_profile(normalized_user_id),
        memberships_repo.get_membership(normalized_user_id),
        courses_repo.list_courses(teacher_id=normalized_user_id, limit=_COURSE_LIMIT),
        courses_repo.list_my_courses(normalized_user_id),
