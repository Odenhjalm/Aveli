...
from ..services import supabase_observability
...
_TOOL_DEFINITIONS = [
    {
        ...
        "description": "Return read-only Supabase database/configuration connection health without exposing secrets.",
        "inputSchema": _READ_ONLY_TOOL_SCHEMA,
    },
    {
        "name": "supabase_auth_state",
        "description": "Return read-only auth substrate and app.auth_subjects alignment diagnostics.",
        "inputSchema": _READ_ONLY_TOOL_SCHEMA,
    },
    {
        "name": "supabase_domain_projection_health",
        "description": "Return read-only counts and projection consistency checks for app-domain tables.",
        "inputSchema": _READ_ONLY_TOOL_SCHEMA,
    },
    {
        "name": "supabase_storage_health",
        "description": "Return read-only storage bucket/object catalog diagnostics and media storage reference checks.",
        "inputSchema": _READ_ONLY_TOOL_SCHEMA,
    },
]
_MCP_SERVER_NAME = "aveli-supabase-observability-mcp"
...
def _ensure_access_allowed(request: Request) -> None:
    if not settings.supabase_observability_mcp_enabled:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Not found")
    if not _client_is_local(request):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Supabase Observability MCP is restricted to local clients",
        )
    if not _origin_is_local(request.headers.get("origin")):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Supabase Observability MCP rejected the supplied Origin header",
        )
...
async def _call_tool(name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    if name in {"supabase_connection_health", "get_supabase_connection_health"}:
        return await supabase_observability.get_connection_health()
    if name in {"supabase_auth_state", "get_supabase_auth_state"}:
        return await supabase_observability.get_auth_state()
    if name in {"supabase_domain_projection_health", "get_supabase_domain_projections"}:
        return await supabase_observability.get_domain_projection_health()
    if name in {"supabase_storage_health", "get_supabase_storage_state"}:
        return await supabase_observability.get_storage_health()
    raise KeyError(name)


@router.get("/mcp/supabase-observability")
async def supabase_observability_mcp_stream(request: Request) -> Response:
    _ensure_access_allowed(request)
    return JSONResponse(
        content=_contract_response(
            status="ok",
            data={},
            query="GET /mcp/supabase-observability",
        )
        | {"confidence": "low"},
...
@router.post("/mcp/supabase-observability")
async def supabase_observability_mcp_endpoint(request: Request) -> Response:
    _ensure_access_allowed(request)
    try:
