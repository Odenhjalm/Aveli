...
void main() {
  ...
  testWidgets(
    'backend canonical cover wins after editor reload when cover_media_id exists',
    (tester) async {
      final studioRepo = _MockStudioRepository();
      final mediaRepo = _MockMediaPipelineRepository();
      var fetchCourseMetaCalls = 0;
      const localPreviewUrl =
          'http://localhost:8080/api/files/public-media/lessons/lesson-1/images/cover.png';
      const canonicalResolvedUrl =
          'http://localhost:8080/api/files/public-media/courses/course-1/canonical-cover.png';
      ...
      when(() => studioRepo.fetchCourseMeta('course-1')).thenAnswer((_) async {
        fetchCourseMetaCalls += 1;
        if (fetchCourseMetaCalls == 1) {
          return {
            'title': 'Testkurs',
            'slug': 'testkurs',
            'description': 'Beskrivning',
        ...
        return {
          ...
          'cover': {
            'media_id': 'cover-1',
            'state': 'ready',
            'resolved_url': canonicalResolvedUrl,
      ...
      when(() => studioRepo.fetchLessonMediaPreviews(any())).thenAnswer(
        (_) async => <String, Map<String, dynamic>>{
          'media-1': {
            'media_type': 'image',
            'resolved_preview_url': localPreviewUrl,
            'authoritative_editor_ready': true,
          },
        },
      );
      ...
      expect(fetchCourseMetaCalls, greaterThanOrEqualTo(2));
      expect(
        find.byWidgetPredicate(
          (widget) =>
              widget is Image &&
              widget.image is NetworkImage &&
              (widget.image as NetworkImage).url == canonicalResolvedUrl,
        ),
        findsOneWidget,
      );
      expect(find.text('Aktiv kursbild'), findsOneWidget);
  ...
  testWidgets(
    'saving course persists selected cover_media_id from editor override',
    (tester) async {
      final studioRepo = _MockStudioRepository();
      final mediaRepo = _MockMediaPipelineRepository();
      ...
      expect(patch['cover_media_id'], 'cover-1');
      expect(patch.containsKey('cover_url'), isFalse);
      expect(patch.containsKey('resolved_url'), isFalse);
    },
  );
}
