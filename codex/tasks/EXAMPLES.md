...
Starta alltid en ny task-branch innan du kor en uppgift:
...
## 2) Larar-Studio - grund
```
/agent Bygg /studio:
- Guard: teacher/admin, annars ansokningsvy som skriver till public.teacher_requests(note)
...
## 3) Stripe-flode
```
/agent Lagg Stripe checkout + Supabase edge webhook:
- start_order() och complete_order()
- PaymentSheet/Checkout i Flutter
- Webhook satter orders.status=paid och triggar uppdateringar
```
